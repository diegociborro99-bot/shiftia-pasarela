...
src/
  ...
  styles/*.css             estilos por área; 01–20 vienen del piloto (tokens, topbar,
                           ...
                           21–26 son de Pasarela (app, equipo, impresión, horas,
                           cierre por fechas, compartir, móvil).
...
| `12-cierre-local.js` | Cierre de un local por fechas (24/09, D11): el visor de tres pasos (días y motivo, qué hace cada persona —apoyo, sin trabajo, vacaciones, día libre—, resumen), editar y reabrir, y el aviso de «Cuándo abre». |
...
- **Persona**: `{id, nombre, puesto, locales, franjas, libra, partido:{dias}, cocina:{titular, reserva, soloDias}, abre:{localId:[franjas]}, noAbre, nuncaCon, nuncaConFlex, nuncaConOff, localHabitual, cubreA, vetos, contrato:{horasSemana}, ausencias:[{tipo,desde,hasta?,franjas?}], prefs, nota, supuestos, color, libraPuntual:[{semana, dias}], libreVariable?, soloCocina?, noPrimero:[franjas], standby?, inactivas?:[claves]}`. Una ausencia sin `franjas` es el día entero; con `['M']` o `['T']`, media jornada (D10); para la nómina, media de SU jornada (`esMediaJornada`). `libraPuntual`: el día libre de una semana concreta, que sustituye al fijo y caduca solo (D9). `inactivas`: las características de la ficha apagadas. Desde el 24/09 (fase 6): «nunca con» es una pareja y está en las dos fichas (`nuncaCon`), con «flexible» (`nuncaConFlex`) y el interruptor (`nuncaConOff`, las parejas apagadas) también por pareja; `localHabitual` es el local habitual elegido (si no, el primero de `locales`); «sin local fijo» es no tener `locales` (ya no hay `comodin`); y «Contrato» no es un interruptor (`inactivas` no lo lleva).
...
- **Cierres por fechas** (24/09, D11): `S.cierresPuntuales = [{id, localId, dias:{iso:[franjas]}, motivo, detalle, decisiones:{pid:{tipo:'REFUERZA'|'SIN'|'LD'|'VAC', destinos?, turnos?}}, retirados, origen?}]`. Fuera de `S.meses` (cruza semanas y meses y sobrevive a Vaciar); nunca en `S.cierres`, que es «mes cerrado para la nómina».
- **Esquema y migraciones**: `S.esquema` (hoy 2, `ESQUEMA_PLANILLA`: el servidor no acepta el guardado de una pestaña con un esquema anterior, 426) y `S.migraciones` (las migraciones de una sola vez ya hechas: `marcasAuto2409`, `abrePatron2409`, `huerfanas2509`…).
...
`generarPlanilla` es determinista: primero retira lo automático que ya no vale (`retirarQueIncumplen`: nunca lo puesto a mano; sale listado en «Qué ha cambiado»), luego lo fijo (`instanciarFijo`: la semana tipo, saltando ausentes y con «cubre a» como primera alternativa, y quien apoya por un cierre en su destino) y luego rellena los mínimos con `candidatosPara` (puntuación con razones; «cubre a» y el apoyo de un cierre van delante, y la sala no se lleva a quien hace falta para una cocina obligatoria de ese día). Devuelve aplicados, huecos con `porQueNadie` (`huecoFaltan`, `huecoCocina`, `huecoPrimero`), coberturas y rechazados.
