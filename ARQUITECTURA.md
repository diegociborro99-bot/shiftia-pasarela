...
- **Local**: `{id, nombre, corto, color, abre:{M:[dows],T:[dows]}, minimos:{M:{dow:n},T:{…}}, supuestos, cocina:{obligatoria, titulares, reservas, posicion}, primero:{M,T}, partidoAbre:{M,T}, horario:{M,T,porDow}, horarioSupuesto, cierreAprox, horarioPartido:{M,T}, horarioPartidoSupuesto, duracion:{M,T}, duracionSupuesta, descansoMin}`. `horarioDe(l, dow, franja, partido)` devuelve el tramo del partido cuando toca; `minutosTurno(l, dow, franja, override, partido)` cuenta por este orden: horario de la casilla, tramo del partido, `duracion` fija (8 h de fábrica) y, si no hay nada, lo que abre el local menos `descansoMin`; `migrarHorarios` pasa las planillas guardadas a los horarios confirmados el 15/09 sin pisar lo editado a mano.
