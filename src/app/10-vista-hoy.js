...
function renderDia() {
  ...
  $('#diaSide').innerHTML = htmlCierresDelDia(iso) + `
    <div class="scard"><span class="micro">Ausentes hoy · ${ausentes.length}</span><div class="lst">${ausentes.map(p => { const a = ausenciaEn(p, iso); return `<div class="srow"><span class="av" style="background:${avColor(p.id)};width:24px;height:24px;font-size:9px">${esc(initials(p.nombre))}</span><span class="nm">${esc(p.nombre)}</span><span class="abschip a-${esc(a.tipo)}">${esc(etiquetaAusencia(a))}</span></div>`; }).join('') || '<div class="szero">Nadie ausente.</div>'}</div></div>
