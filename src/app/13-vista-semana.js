...
function renderSemana() {
  ...
  const celda = (c, tid) => {
    ...
    const bloques = posicionesDe(S, S.staff, c.e, c.iso, tid).map(x => x.hueco ? `<button class="wav whueco" data-pick="${c.iso}|${tid}" data-tipstr="${esc('Hueco disponible: ' + (x.motivo || ''))}" aria-label="Hueco disponible">1·?</button>` : `<button class="wav${x.abre ? ' abre' : ''}${x.abreNoApto ? ' abrewarn' : ''}${x.cocina ? ' cocina' : ''}${x.forzado ? ' forzado' : ''}${x.comodin ? ' comodin' : ''}" style="--pc:${avColor(x.pid)}" data-wpers="${c.iso}|${tid}|${x.pid}" data-tipstr="${esc(x.pos + '. ' + x.nombre + (x.abre ? (x.abreFijo ? ' · sale el primero (fijo)' : ' · abre') : '') + (x.abreNoApto ? ' · ⚠ ' + textoAbreNoApto(x.abreNoApto) : '') + (x.cocina ? ' · cocina' : '') + (x.continuo ? ' · turno continuo' : x.partido ? ' · partido' : '') + (x.por ? ' · por ' + nombrePid(x.por) : '') + (x.nota ? ' · ' + x.nota : ''))}">${esc(nombreCorto(x.nombre))}${x.partido ? '<sup>P</sup>' : x.continuo ? '<sup>C</sup>' : ''}${x.tramo ? `<small class="wtr">${esc(horaCorta(x.tramo.ini))}–${esc(horaCorta(x.tramo.fin))}</small>` : ''}</button>`).join('');
