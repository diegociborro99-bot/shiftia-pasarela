...
function renderSemana() {
  ...
  const celda = (c, tid) => {
    ...
    const bloques = lista.map((x, i) => `<button class="wav${x.abre ? ' abre' : ''}${x.cocina ? ' cocina' : ''}${x.forzado ? ' forzado' : ''}" style="--pc:${avColor(x.pid)}" data-wpers="${c.iso}|${tid}|${x.pid}" data-tipstr="${esc((i + 1) + '. ' + nombrePid(x.pid) + (x.abre ? ' · abre' : '') + (x.cocina ? ' · cocina' : '') + (x.razon ? '\n' + x.razon : '') + (x.avisos && x.avisos.length ? '\naviso: ' + x.avisos.join(', ') : ''))}">${esc(initials(nombrePid(x.pid)))}</button>`).join('');
