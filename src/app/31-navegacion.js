...
function activarModoEmpleado() {
  ...
  for (const d of est.days) {
    ...
    const partidoDia = cas.some(c => partirTurno(c.tid).franja === 'M') && cas.some(c => partirTurno(c.tid).franja === 'T');
    dias += `<div class="festrow${d.iso === hoy ? ' f-manual' : ''}" style="border-left-color:${cas.length ? colorLocal(partirTurno(cas[0].tid).localId) : 'var(--warn)'}"><span class="festinfo"><b>${fmtLargo(d.iso)}${d.iso === hoy ? ' · hoy' : ''}${d.festivo ? ' · festivo' : ''}</b>${aus ? `<small>${esc((AUS_LBL[aus.tipo] || {}).label || aus.tipo)}${aus.detalle ? ' · ' + esc(aus.detalle) : ''}</small>` : ''}${cas.map(c => { const { localId, franja } = partirTurno(c.tid); const l = localDe(S, localId); const hr = c.entry.ini && c.entry.fin ? c.entry : horarioDe(l, d.dow, franja, partidoDia && !c.entry.abre); return `<small><span class="lpill" style="--lc:${colorLocal(localId)}">${esc(l ? l.corto : localId)}</span> ${FRANJA_LBL[franja].toLowerCase()}${hr ? ` ${esc(hr.ini)}–${esc(hr.fin)}` : ''}${c.entry.abre ? ' · <b>abres</b>' : ''}${c.entry.cocina ? ' · cocina' : ''}${evPor[d.iso] ? ' · ⚽ ' + esc(evPor[d.iso].map(x => x.nombre).join(', ')) : ''}</small>`; }).join('')}</span></div>`;
  }
