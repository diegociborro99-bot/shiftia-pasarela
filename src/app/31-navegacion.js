...
function activarModoEmpleado() {
  ...
  for (const d of est.days) {
    ...
    const cies = FRANJAS.map(f => ({ f, dc: decisionCierre(S, p.id, d.iso, f) })).filter(x => x.dc);
    // de apoyo «donde haga falta» y aún sin sitio ese día: que no crea que libra (revisión F2)
    const sinSitio = cies.length ? apoyosSinSitio(S, S.staff, est, d.iso).filter(x => x.pid === p.id).map(x => x.franja) : [];
    const cieTxt = cies.map(({ f, dc }) => { const dd = DECISIONES_CIERRE.find(x => x.id === dc.tipo); const m = motivoCierreTxt(dc.cierre); return `<small class="cieemp"><span class="lpill" style="--lc:${colorLocal(dc.cierre.localId)}">${esc((localDe(S, dc.cierre.localId) || {}).corto || dc.cierre.localId)}</span> ${esc(nombreLocal(dc.cierre.localId))} · ${FRANJA_LBL[f].toLowerCase()}: cerrado${m ? ' por ' + esc(m) : ''} · <b>${esc(dd ? dd.largo : dc.tipo)}</b>${sinSitio.includes(f) ? ' · aún sin sitio: te lo dirá el encargado' : ''}</small>`; }).join('');
    if (!cas.length && !aus && !cies.length) continue;
    const rep = turnoDelDia(S, est, d.iso, p.id);   // partido, continuo y la franja que abre
    dias += `<div class="festrow${d.iso === hoy ? ' f-manual' : ''}" style="border-left-color:${cas.length ? colorLocal(partirTurno(cas[0].tid).localId) : 'var(--warn)'}"><span class="festinfo"><b>${fmtLargo(d.iso)}${d.iso === hoy ? ' · hoy' : ''}${d.festivo ? ' · festivo' : ''}</b>${aus ? `<small>${esc((AUS_LBL[aus.tipo] || {}).label || aus.tipo)}${aus.detalle ? ' · ' + esc(aus.detalle) : ''}</small>` : ''}${cieTxt}${cas.map(c => { const { localId, franja } = partirTurno(c.tid); const l = localDe(S, localId); const hr = c.entry.ini && c.entry.fin ? c.entry : horarioDe(l, d.dow, franja, rep.enPartido, rep.abre); return `<small><span class="lpill" style="--lc:${colorLocal(localId)}">${esc(l ? l.corto : localId)}</span> ${FRANJA_LBL[franja].toLowerCase()}${hr ? ` ${esc(hr.ini)}–${esc(hr.fin)}` : ''}${c.entry.abre ? ' · <b>abres</b>' : ''}${c.entry.cocina ? ' · cocina' : ''}${evPor[d.iso] ? ' · ⚽ ' + esc(evPor[d.iso].map(x => x.nombre).join(', ')) : ''}</small>`; }).join('')}</span></div>`;
  }
