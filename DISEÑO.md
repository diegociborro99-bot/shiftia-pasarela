...
**La ficha empieza por fecha, nombre, teléfono, edad y zona** (Diego, 17/09): los campos marcados `cabecera` en `CAMPOS_ENTREVISTA` suben arriba del todo y salen del bloque de la entrevista. **La fecha se pone sola** al registrar a alguien (`fmtLargo(isoHoy())`) y va la primera de todo, tanto en la ficha como en el perfil. Como se rellena sola, va marcada `meta: true` y no cuenta para «entrevista contestada» — si no, cualquier alta recién creada figuraría como contestada.
