...
**La fecha se pone sola** al registrar a alguien (`fmtLargo(isoHoy())`) y va **la primera de todo**, tanto en la ficha como en el perfil: es el primer campo de `CAMPOS_ENTREVISTA`. Como se rellena sola, va marcada `meta: true` y no cuenta para «entrevista contestada» — si no, cualquier alta recién creada figuraría como contestada.
